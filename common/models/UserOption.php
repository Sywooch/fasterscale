...
class UserOption extends \yii\db\ActiveRecord
{
  ...
  public static function getPastCheckinDates($as_arr = false, $zero_month_idxed = false)
  {
    $past_checkin_dates = [];
    $query = new Query;
    ...
    foreach($temp_dates as $temp_date) {
      $temp_date = Time::convertUTCToLocal($temp_date['date'], false);

      if($as_arr) {
        $temp_date = explode("-", $temp_date);
        $temp_date = array_map('intval', $temp_date);
        if($zero_month_idxed) {
          $temp_date[1] = $temp_date[1] - 1;
        }
      }

      $past_checkin_dates[] =  $temp_date;
    }
